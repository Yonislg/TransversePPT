...
% Flags
imeq=1; %Include momentum equation
if imeq
...
ue_guess = sqrt(Te/m_i)+(gem_guess -  ge(ne_0, Te, -varphi_sf*Te/e))/ne_0
if imeq
...
[V_C, V_A, geC_em, geA_em, E_wc, E_wa, uxe, phi_B, phi_D,fx] = currentsolver(plasma_properties,design_parameters, initial_state, phi_A, phi_C);

function [V_C, V_A, geC_em, geA_em, E_wc, E_wa, uxe, phi_B, phi_D,fx] = currentsolver(plasma_properties,design_parameters, initial_state, phi_A, phi_C)
    global e imeq;
...
function jxe_v2 = total_current2(x, plasma_properties,design_parameters, phi_A, phi_C)
    global m_i mu_0 e u_ze gi imeq;
    [Te, ne_0, ui0] = deal(plasma_properties{:});
    [T_wka, T_wkc, E_i, A_G, h, L, W] = deal(design_parameters{:});

    jxe_v2(1) = wall_e_field(T_wkc, x(1),x(3),ne_0, Te, ui0) - x(5); % Cathode electric field
    jxe_v2(2) = - x(3) + SEE(gi, E_i, W) + schottky(T_wkc, W, x(5), A_G); %Cathode lectron emissions
    if imeq
        jxe_v2(3) = (x(4) - ge(ne_0, Te, -x(2)*Te/e)  - x(3) + ge(ne_0, Te, -x(1)*Te/e))/2- ne_0*x(7);
    else
        jxe_v2(3) = (x(4) - ge(ne_0, Te, -x(2)*Te/e)  - x(3) + ge(ne_0, Te, -x(1)*Te/e))/2;
    end
    jxe_v2(4) = -2*ne_0*sqrt(Te/m_i) + ge(ne_0, Te, -x(2)*Te/e)+ ge(ne_0, Te, -x(1)*Te/e)- x(4)- x(3);
    jxe_v2(5) =  - x(4)  + SEE(gi, E_i, W)+ schottky(T_wka, W, x(6), A_G); % Anode electron emissions
    jxe_v2(6) = wall_e_field(T_wka, x(2),x(4),ne_0, Te, ui0) -x(6);  %Anode electric field
    if imeq
        jxe_v2(7) = -(phi_A-x(1)*Te/e-(phi_C+x(2)*Te/e))/h+ u_ze*(e*ne_0*L*x(7)*mu_0); % Bulk current
    end
end
