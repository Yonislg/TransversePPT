...
% end
figure(1)
title("Characteristic Curve for Different n_{e0}")
ylabel("Particle Flux [m^{-3}s^{-1}]")
xlabel("Potential Drop [V]") 
hold on
for j = 1:3
     y1(j,:)= ge_bolz((2*j-1)*10^22,1*e,x);
     plot(x,y1(j,:),'DisplayName',strcat(int2str(2*j-1),"\cdot10^{22}m^{-3}"))
end
legend
...
%legend("1","3","5")
figure(2)
title("Characteristic Curve for Different T_e")
ylabel("Particle Flux [m^{-3}s^{-1}]")
xlabel("Potential Drop [V]") 
hold on
for i = 1:3
     y2(i,:)= ge_bolz(10^22, i*e,x);
     plot(x,y2(i,:),'DisplayName',strcat(int2str(i)," eV"))
end
legend
...
title("Numerical Derivative for n_{e0}")
...
title("Numerical Derivative for T_e")
ylabel("\Delta Flux / \Delta V with \Delta V = 1")
xlabel("Potential Drop [V]") 
legend(H2.String)